...
export default function EditStudentPage({ params }: { params: { id: string } }) {
    ...
    const fetchStudent = async () => {
      ...
      if (studentData) {
        ...
        form.reset({
            ...
            assignmentsScore: studentData.assignmentsScore,
            participationScore: studentData.participationScore,
            extraCurricularScore: studentData.extraCurricularScore,
        });
      } else {
        notFound();
      }
  ...
  const onSubmit = async (data: StudentFormValues) => {
    setLoading(true);
    try {
      const result = await updateStudentAction(params.id, data);
      ...
      if (result.success) {
        ...
        router.push(`/dashboard/student/${params.id}`);
      } else {
  ...
  return (
    <Card>
      <CardHeader>
        <CardTitle className="font-headline text-2xl">Edit {student?.name}</CardTitle>
